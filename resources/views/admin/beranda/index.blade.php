        ...
        @foreach ($skema as $asu)
                                ...
                                <div class="card-body col-md-6">
                                    <form action="{{ route('sett-beranda.update', $asu->id) }}" method="POST"
                                        ...
                                        @csrf
                                        @method('put')
                                        <label class="text-primary font-weight-bold" for="file"><i class="fas fa-images"></i> Input Banner Skema</label>
        ...
        @foreach ($tuk as $asu)
                            <div class="card-body col-md-6">
                                <form action="{{ route('sett-beranda.update', $asu->id) }}" method="POST"
                                    ...
                                    @csrf
                                    @method('put')
                                    <label class="text-primary font-weight-bold" for="file"><i class="fas fa-images"></i> Input Banner TUK</label>
                                    <input type="file" class="form-control" name="image" accept=".png, .jpg, .jpeg" id="file">
                                    <br>
                                    <button type="submit" class="btn btn-info btn-block"><i class="fas fa-save"></i>
                                        Update</button><br>
